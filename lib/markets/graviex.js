...
var base_url = 'https://graviex.net/api/v3/';
function get_summary(coin, exchange, graviex_id, cb) {
    var summary = {};
    request({ uri: base_url + '/markets?market_id=' + graviex_id, json: true }, function (error, response, body) {
        if (error) {
            ...
            summary['high'] = body.Data['HighPrice'].toFixed(8);
            summary['low'] = body.Data['LowPrice'].toFixed(8);
...
function get_trades(coin, exchange, graviex_id, cb) {
    return cb(null, []);

    // var req_url = base_url + '/GetMarketHistory/' + graviex_id;
...
function get_orders(coin, exchange, graviex_id, cb) {
    var req_url = base_url + '/getorderbook?market_id=' + graviex_id;
    request({ uri: req_url, json: true }, function (error, response, body) {
        if (body.success == "1") {
...
module.exports = {
    get_data: function (coin, exchange, graviex_id, cb) {
        var error = null;
        get_orders(coin, exchange, graviex_id, function (err, buys, sells) {
            if (err) { error = err; }
            get_trades(coin, exchange, graviex_id, function (err, trades) {
                if (err) { error = err; }
                get_summary(coin, exchange, graviex_id, function (err, stats) {
                    if (err) { error = err; }
